...
IFS=$'\n' COMMANDS=(
	"dev:turbo|🚀 Starting the full-stack turbo server - (development)|doppler run -- pnpm dev"
	"dev:client|💻 Staring the NextJs Client - (development)|doppler run -- pnpm dev --filter=client"
	"dev:server|⚙️ Starting the HonoJs Server - (development)|doppler run -- pnpm dev --filter=server"
	"db:studio|🗃️ Opening Prisma Studio in the browser|cd ./packages/db && doppler run -- pnpm db:studio"
	"db:pull|🔽 Pulling schema from the remote database|cd ./packages/db && doppler run -- pnpm db:pull"
	"db:generate|⚡ Generating the Prisma client|cd ./packages/db && doppler run -- pnpm db:generate"
	"db:push|🔼 Pushing schema changes to the database|cd ./packages/db && doppler run -- pnpm db:push"
	"db:drop|🔥 Droping the database (irreversible)|cd ./packages/db && doppler run -- pnpm db:drop"
	"worker:start:git|👷 Starting the Git Extract Worker|cd ./apps/server && doppler run -- pnpm worker:git"
	"worker:start:container|📦 Starting the Container Worker|cd ./apps/server && doppler run -- pnpm worker:container"
	"worker:start:file-sync|🔄 Starting the File Sync Worker|cd ./apps/server && doppler run -- pnpm worker:file-sync"
	"worker:clear:extract|🧹 Clearing the 'extract' queue|cd ./apps/server && doppler run -- pnpm clear:queue extract"
	"worker:clear:container|🧹 Clearing the 'container' queue|cd ./apps/server && doppler run -- pnpm clear:queue container"
	"worker:clear:file-sync|🧹 Clearing the 'file-sync' queue|cd ./apps/server && doppler run -- pnpm clear:queue file-sync"
)
...
run_command() {
	...
	if [[ "$cmd" == *"pnpm dev"* || "$cmd" == *"worker"* || "$cmd" == *"studio"* ]]; then
		echo
