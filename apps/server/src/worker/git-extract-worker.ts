...
socketClient.on("connect", () => {
  console.log("🤖 Extract Worker connected to server via WebSocket:", socketClient.id);
});

socketClient.on("connect_error", (err) => {
  console.error("❌ Extract Extract Worker failed to connect to server via WebSocket:", err);
});
